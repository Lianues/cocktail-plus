...
var PNG_CHUNK_HEADER_BYTES = 8;
var DEFAULT_SCAN_CONCURRENCY = 6;
function resolveScanConcurrency(config2) {
  const raw = Number(config2?.charactersAllScanConcurrency);
...
async function calculateChatSize(chatsRoot, avatarFileName) {
  ...
  try {
    ...
    const entries2 = await fs4.promises.readdir(charDir, { withFileTypes: true }).catch(() => null);
    if (!entries2) return { chatSize, dateLastChat };
    await Promise.all(entries2.map(async (entry) => {
      if (!entry.isFile()) return;
      try {
        const stat = await fs4.promises.stat(path3.join(charDir, entry.name));
        chatSize += stat.size;
...
async function fetchCharactersAllDirect(ctx, config2, options = {}) {
  ...
  const concurrency = resolveScanConcurrency(config2);
  const characters = [];
  let processed = 0;
...
var REQUEST_COMPRESSION_DEFAULTS = Object.freeze({
  enabled: false,
  minPayloadSize: "256kb",
  maxPayloadSize: "8mb",
  timeout: 4e3
});
var stConfigFallbackCache = null;
...
function getPathValue2(object, key, fallback = void 0) {
  try {
    const parts = String(key).split(".");
    let current = object;
    for (const part of parts) {
      if (current === null || current === void 0 || !Object.prototype.hasOwnProperty.call(current, part)) return fallback;
...
function parseSimpleYaml(text) {
  ...
  for (const line of lines) {
    ...
    if (rawValue === void 0 || stripInlineComment(rawValue).trim() === "") {
      const child = {};
      parent[key] = child;
...
function getRuntimeConfigValue(key, defaultValue = null, typeConverter = null) {
  ...
  let value = Object.prototype.hasOwnProperty.call(process.env, envKey) ? process.env[envKey] : getPathValue2(getFallbackConfig(), key, defaultValue);
