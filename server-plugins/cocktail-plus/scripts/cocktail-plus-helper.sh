...
const schema = [
  ...
  ['asyncCharactersAllOnMiss', 'characters 无缓存时异步构建并先进入页面', 'bool', true],
  ['allowEmptyCharactersAllOnMiss', '允许无缓存先返回空角色列表（配合自动加载保护）', 'bool', true],
  ['blockCharactersAllOnMiss', '阻塞等待真实 characters/all（慢速兼容模式）', 'bool', false],
  ...
  ['serviceWorkerFastRouteFallback', 'SW 兜底 /version 与 characters/all', 'bool', true],
  ...
  ['patchParallelActivateExtensions', '并行激活扩展（等待完成，不延后到 APP_READY）', 'bool', true],
  ['deferExtensionActivationUntilAppReady', 'APP_READY 后激活前端扩展（最快首屏，带事件回放）', 'bool', true],
  ['browserLogCaptureEnabled', '接管浏览器 console/error 日志到后端', 'bool', true],
